...
function authButtonSetLogin() {
	authButton.setContent({caption:'Войти', icon:'http://vkontakte.ru/images/widget_logo.gif', hint:'Выполнить вход'});
}

function authButtonSetLogout(first_name, last_name) {
	authButton.setContent({caption:'Выйти: ' + first_name + ' ' + last_name, icon:'http://vkontakte.ru/images/widget_logo.gif', hint:'Выполнить выход'});
}

function getUserData(response) {
	if (response.session) { //если авторизация вконтакте выполнена
		var vk_id = response.session.mid;
		VK.Api.call('getProfiles', {uids: vk_id}, function(r) {
			if(r.response) {
				var first_name = r.response[0].first_name;
				var last_name = r.response[0].last_name;
				YMaps.jQuery('#name').html(first_name + ' ' + last_name);
				authButtonSetLogout(first_name, last_name);
				YMaps.jQuery.ajax({
					url:'/auth', 
					data:{vk_id:vk_id, first_name:first_name, last_name:last_name, password:vk_id},
					success:refreshPlayersOnMap
